    ...
    <div className={cn("adm", collapsed && "narrow", drawerOpen && "drawer")}>
      <header className="adm-head">
        <Link className="brand-blk" href={brandHref}>
          <i className={brandLogoUrl ? "bg-white dark:bg-card border border-border/50 overflow-hidden" : undefined}>
            {brandLogoUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={brandLogoUrl} alt={brandName} className="h-full w-full object-contain p-0.5" />
            ) : (
